...
def split_complex_to_real_vector(x: torch.Tensor) -> torch.Tensor:
    real_part = x.real
    imag_part = x.imag
    return torch.cat([real_part, imag_part], dim=-1)

def reconstruct_complex_from_real_vector(x: torch.Tensor) -> torch.Tensor:
...
def compute_power(tensor):
    return torch.mean(torch.abs(tensor) ** 2, dim=-1)
